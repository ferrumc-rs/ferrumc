...
impl NetEncode for String {
    ...
    where
        W: AsyncWrite + Unpin,
    {
        self.as_str().net_encode(writer, encode_option).await?;
        Ok(())
    }
}
...
impl<'a> NetEncode for &'a str {
        ...
        W: AsyncWrite + Unpin,
    {
        ...
        len.net_encode(writer, encode_option).await?;
...
impl<E: NetEncode> NetEncode for Vec<E> {
        ...
        W: AsyncWrite + Unpin,
    {
        ...
        for v in self {
            v.net_encode(writer, encode_option).await?;
        }
...
impl<O: NetEncode> NetEncode for Option<O> {
    ...
    {
        match self {
            Some(v) => v.net_encode(writer, encode_option).await,
            None => Ok(()),
        }
    }
}
