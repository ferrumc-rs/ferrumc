...
use crate::inventory::update::{InventoryUpdate, INVENTORY_UPDATES_QUEUE};
...
mod tests {
    use super::*;
    // --- 1. Import the Core Type directly ---
    use ferrumc_core::items::item_id::ItemID;

    // --- 2. Helper: No more VarInt needed ---
    fn make_slot_with_id(id: i32) -> InventorySlot {
        InventorySlot {
            item_id: Some(ItemID::new(id)),
            count: 1, // Default count to 1 for valid items
            ..Default::default()
        }
    }
    ...
    fn test_add_and_get_item() {
        ...
        // Check slot 0
        let item0 = inv.get_item(0).unwrap();
        assert!(item0.is_some());
        assert_eq!(item0.as_ref().unwrap().item_id.unwrap().inner(), 1);
    ...
    fn test_invalid_index() {
        ...
        // Index 2 is out of bounds for size 1
        assert!(matches!(
            inv.get_item(2),
