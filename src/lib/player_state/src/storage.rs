...
use ferrumc_storage::{errors::StorageError, lmdb::LmdbBackend};
...
impl PlayerStateStorage {
    ...
    pub fn save_player_state(&self, state: &PlayerData) -> Result<(), PlayerDataError> {
        if !self.storage_backend.table_exists(TABLE_NAME.to_string())? {
            self.storage_backend.create_table(TABLE_NAME.to_string())?;
        }
        let key = state.uuid;
        ...
        self.storage_backend
            .upsert(TABLE_NAME.to_string(), key, encoded)?;
        Ok(())
    }

    pub fn load_player_state(&self, key: u128) -> Result<Option<PlayerData>, PlayerDataError> {
        if let Some(bytes) = self.storage_backend.get(TABLE_NAME.to_string(), key)? {
