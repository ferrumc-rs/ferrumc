...
pub fn derive(input: TokenStream) -> TokenStream {
    ...
    let serialize_async_impl = match &input.data {
        Data::Struct(data_struct) => {
            ...
            let fields = fields.iter().enumerate().map(|(i, field)| {
                ...
                if flatten {
                    return quote! {
                        <#ty as ::ferrumc_nbt::NBTSerializable>::serialize_async(&self.#ident, writer, &::ferrumc_nbt::NBTSerializeOptions::Flatten).await;
                    };
                }
                ...
                if let Some(condition) = skip_if {
                    quote! {
                        if !#condition (&self.#ident) {
                            <#ty as ::ferrumc_nbt::NBTSerializable>::serialize_async(&self.#ident, writer, &::ferrumc_nbt::NBTSerializeOptions::WithHeader(#serialize_name)).await;
                        }
                    }
                } else {
                    quote! {
                ...
                let serialize_fields = match &variant.fields {
                    Fields::Named(fields_named) => {
                        let fields = fields_named.named.iter().map(|field| {
                            ...
                            quote! {
                                <#ty as ::ferrumc_nbt::NBTSerializable>::serialize_async(#ident, writer, &::ferrumc_nbt::NBTSerializeOptions::WithHeader(#serialize_name)).await;
                            }
                        });
                        ...
                        let tagged = if let Some(tag) = tagged {
                            if untagged { fields } else {
                                quote! {
                                    <&'_ str as ::ferrumc_nbt::NBTSerializable>::serialize_async(&#tag_name, writer, &::ferrumc_nbt::NBTSerializeOptions::WithHeader(#tag)).await;
                                    <u8 as ::ferrumc_nbt::NBTSerializable>::serialize_async(&10, writer, &::ferrumc_nbt::NBTSerializeOptions::None).await;
                                    <&'_ str as ::ferrumc_nbt::NBTSerializable>::serialize_async(&#variant_content, writer, &::ferrumc_nbt::NBTSerializeOptions::None).await;
                                    #fields
                                    <u8 as ::ferrumc_nbt::NBTSerializable>::serialize_async(&0u8, writer, &::ferrumc_nbt::NBTSerializeOptions::None).await;
                                }
                            }
                        } else { fields };
                    ...
                    Fields::Unnamed(fields_unnamed) => {
                        let fields = fields_unnamed.unnamed.iter().enumerate().map(|(i, field)| {
                            ...
                            if !untagged && tagged.is_some() {
                                if fields_unnamed.unnamed.len() == 1 {
                                    quote! {
                                        <#ty as ::ferrumc_nbt::NBTSerializable>::serialize_async(#ident, writer, &::ferrumc_nbt::NBTSerializeOptions::WithHeader(#variant_content)).await;
                                    }
                                } else {
                                    quote! { unimplemented!(); }
                                }
                                ...
                                quote! {
                                    <#ty as ::ferrumc_nbt::NBTSerializable>::serialize_async(#ident, writer, &::ferrumc_nbt::NBTSerializeOptions::None).await;
                                }
                            }
                        });
                    ...
                    Fields::Unit => match tagged {
                        Some(tag) => {
                            if untagged {
                                quote! {
                                    => {
                                        <&'_ str as ::ferrumc_nbt::NBTSerializable>::serialize_async(&#tag_name, writer, &::ferrumc_nbt::NBTSerializeOptions::None).await;
                                    }
                                }
                            } else {
                                quote! {
    ...
    let expanded = quote! {
        impl #impl_generics ::ferrumc_nbt::NBTSerializable for #name #ty_generics #where_clause {
            fn serialize<W: std::io::Write>(&self, writer: &mut W, options: &::ferrumc_nbt::NBTSerializeOptions) {
                match options {
            ...
            async fn serialize_async<'a, W: ::ferrumc_nbt::tokio::io::AsyncWrite + ::core::marker::Unpin>(&self, writer: &mut W, options: &::ferrumc_nbt::NBTSerializeOptions<'a>) {
                match options {
                    ::ferrumc_nbt::NBTSerializeOptions::WithHeader(name) => {
                        <u8 as ::ferrumc_nbt::NBTSerializable>::serialize_async(&Self::id(), writer, &::ferrumc_nbt::NBTSerializeOptions::None).await;
                        <&'a str as ::ferrumc_nbt::NBTSerializable>::serialize_async(&name, writer, &::ferrumc_nbt::NBTSerializeOptions::None).await;
                    }
                    ::ferrumc_nbt::NBTSerializeOptions::Network => {
                        <u8 as ::ferrumc_nbt::NBTSerializable>::serialize_async(&Self::id(), writer, &::ferrumc_nbt::NBTSerializeOptions::None).await;
                    }
