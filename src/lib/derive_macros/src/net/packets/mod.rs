...
pub fn bake_registry(input: TokenStream) -> TokenStream {
            ...
            for attr in item_struct.attrs {
                ...
                match_arms.push(quote! {
                    (#packet_id, #state) => {
                        ...
                        // <#struct_path as ferrumc_net_packets::IncomingPacket>::handle(packet, conn_id).await?;
    ...
    let output = quote! {
        pub async fn handle_packet(packet_id: u8, conn_id: usize, conn_state: crate::connection::ConnectionState, cursor: &mut std::io::Cursor<&[u8]>) -> crate::NetResult<()> {
