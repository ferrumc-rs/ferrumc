...
impl Compressor {
    ...
    pub fn compress(&self, data: &[u8]) -> Result<Vec<u8>, StorageError> {
        match self.algorithm {
            CompressorType::Gzip => {
                compress_gzip(self.level, data)
            }
            CompressorType::Zstd => {
                compress_zstd(self.level, data)
            }
            CompressorType::Brotli => {
                compress_brotli(self.level, data)
            }
            CompressorType::Deflate => {
                compress_deflate(self.level, data)
            }
            CompressorType::Zlib => {
                compress_zlib(self.level, data)
            }
        }
    }
