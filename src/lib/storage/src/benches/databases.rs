...
pub fn database_benchmarks(c: &mut Criterion) {
    ...
    let _handle = runtime.enter();
    {
        ...
        let mut sled_backend = runtime.block_on(async {
            ...
            db_file.push("test.db");
            let backend = ferrumc_storage::backends::sled::SledBackend::initialize(Some(db_file.clone())).await.unwrap();
            temps.push(db_file);
            backend
        });
        ...
        let mut rocksdb_backend = runtime.block_on(async {
            ...
            db_file.push("test.db");
            let mut backend = ferrumc_storage::backends::rocksdb::RocksDBBackend::initialize(Some(db_file.clone())).await.unwrap();
            temps.push(db_file);
            backend.create_table("test".to_string()).await.unwrap();
            backend
        });
        ...
        write_group.bench_with_input("Sled", &("test".to_string(), data.clone()), |b, (table, data)| {
            b.iter(|| {
                runtime.block_on(sled_backend.insert(table.clone(), gen_key(), data.clone())).unwrap();
            });
        });
        write_group.bench_with_input("RocksDB", &("test".to_string(), data.clone()), |b, (table, data)| {
            b.iter(|| {
                runtime.block_on(rocksdb_backend.upsert(table.clone(), gen_key(), data.clone())).unwrap();
            });
        });
        write_group.finish();
        ...
        for _ in 0..READ_KEYS {
            let key = random_key();
            runtime.block_on(redb_backend.insert("test".to_string(), key, data.clone())).unwrap();
            runtime.block_on(surreal_backend.insert("test".to_string(), key, data.clone())).unwrap();
            runtime.block_on(sled_backend.insert("test".to_string(), key, data.clone())).unwrap();
            runtime.block_on(rocksdb_backend.insert("test".to_string(), key, data.clone())).unwrap();
            keys.push(key);
        }
        ...
        read_group.bench_with_input("Sled", &("test".to_string()), |b, (table)| {
            b.iter(|| {
                let key = keys.choose(&mut rand::thread_rng()).unwrap();
                black_box(runtime.block_on(sled_backend.get(table.clone(), *key)).unwrap());
            });
        });
        read_group.bench_with_input("RocksDB", &("test".to_string()), |b, (table)| {
            b.iter(|| {
                let key = keys.choose(&mut rand::thread_rng()).unwrap();
                black_box(runtime.block_on(rocksdb_backend.get(table.clone(), *key)).unwrap());
            });
        });
