...
impl DatabaseBackend for RocksDBBackend {
        ...
        Self: Sized
    {
        ...
        if let Some(path) = store_path {
            let db = DB::open(&options, path).map_err(|e| StorageError::DatabaseInitError(e.to_string()))?;
    ...
    async fn insert(&mut self, table: String, key: u64, value: Vec<u8>) -> Result<(), StorageError> {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            db.put_cf(cf, key.to_be_bytes(), &value).map_err(|e| StorageError::WriteError(e.to_string()))?;
            Ok::<(), StorageError>(())
    ...
    async fn get(&mut self, table: String, key: u64) -> Result<Option<Vec<u8>>, StorageError> {
        ...
        let result = tokio::task::spawn_blocking(move || {
            let db = db.read();
            let cf = db.cf_handle(&table).unwrap();
            let value = db.get_cf(cf, key.to_be_bytes()).map_err(|e| StorageError::ReadError(e.to_string()))?;
            if let Some(value) = value {
                Ok(Some(value.to_vec()))
            } else {
                Ok(None)
            }
        }).await.expect("Failed to get data from database")?;
        Ok(result)
    }
    ...
    async fn delete(&mut self, table: String, key: u64) -> Result<(), StorageError> {
        ...
        tokio::task::spawn_blocking(move || {
            let db = db.read();
            let cf = db.cf_handle(&table).unwrap();
            db.delete_cf(cf, key.to_be_bytes()).map_err(|e| StorageError::WriteError(e.to_string()))?;
            Ok::<(), StorageError>(())
        }).await.expect("Failed to delete data from database")?;
        Ok(())
    }
    ...
    async fn update(&mut self, table: String, key: u64, value: Vec<u8>) -> Result<(), StorageError> {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            db.put_cf(cf, key.to_be_bytes(), &value).map_err(|e| StorageError::WriteError(e.to_string()))?;
            Ok::<(), StorageError>(())
        }).await.expect("Failed to update data in database")?;
        Ok(())
    }
    ...
    async fn upsert(&mut self, table: String, key: u64, value: Vec<u8>) -> Result<bool, StorageError> {
        ...
        }).await.expect("Failed to upsert data in database")?;
        Ok(result)
    }
    ...
    async fn exists(&mut self, table: String, key: u64) -> Result<bool, StorageError> {
        ...
        let result = tokio::task::spawn_blocking(move || {
            ...
            let value = db.get_cf(cf, key.to_be_bytes()).map_err(|e| StorageError::ReadError(e.to_string()))?;
            Ok(value.is_some())
        }).await.expect("Failed to check if key exists in database")?;
        Ok(result)
    }
    ...
    async fn batch_insert(&mut self, table: String, data: Vec<(u64, Vec<u8>)>) -> Result<(), StorageError> {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            db.write(batch).map_err(|e| StorageError::WriteError(e.to_string()))?;
            Ok::<(), StorageError>(())
        }).await.expect("Failed to batch insert data into database")?;
        Ok(())
    }
    ...
    async fn batch_get(&mut self, table: String, keys: Vec<u64>) -> Result<Vec<Option<Vec<u8>>>, StorageError> {
        ...
        let result = tokio::task::spawn_blocking(move || {
            ...
            Ok(values)
        }).await.expect("Failed to batch get data from database")?;
        Ok(result)
    }

    async fn flush(&mut self) -> Result<(), StorageError> {
        
        self.db.read().flush().map_err(|e| StorageError::WriteError(e.to_string()))?;
        Ok(())
    }

    async fn create_table(&mut self, table: String) -> Result<(), StorageError> {
        self.db.write().create_cf(&table, &rocksdb::Options::default()).map_err(|e| StorageError::WriteError(e.to_string()))?;
        Ok(())
    }

    async fn close(&mut self) -> Result<(), StorageError> {
        self.flush().await?;
        self.db.read().flush_wal(true).map_err(|e| StorageError::WriteError(e.to_string()))?;
        Ok(())
    }
}