...
impl DatabaseBackend for RedbBackend {
    async fn initialize(store_path: Option<PathBuf>) -> Result<Self, StorageError> {
        if let Some(path) = store_path {
            let db = if path.exists() {
                redb::Database::open(path).map_err(|e| StorageError::DatabaseInitError(e.to_string()))?
            } else { 
                redb::Database::create(path).map_err( |e| StorageError::DatabaseInitError(e.to_string()))?
            };
    ...
    async fn insert(&mut self, table: String, key: u64, value: Vec<u8>) -> Result<(), StorageError> {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            {
                let tx = db.read().begin_write().map_err(|e| StorageError::WriteError(e.to_string()))?;
                {
                    let mut open_table = tx.open_table(table_def).map_err(|e| StorageError::WriteError(e.to_string()))?;
                                       
                    open_table.insert(key, value.as_slice()).map_err(|e| StorageError::WriteError(e.to_string()))?;
                
                }
                tx.commit().map_err(|e| StorageError::CommitError(e.to_string()))?;
                Ok::<(), StorageError>(())
            }
        }).await.expect("Failed to insert data into database")?;
        Ok(())
    }
    ...
    async fn get(&mut self, table: String, key: u64) -> Result<Option<Vec<u8>>, StorageError> {
        ...
        let result = tokio::task::spawn_blocking(move || {
            let table_def: TableDefinition<u64, &[u8]> = TableDefinition::new(&table);
            let tx = db.read().begin_read().map_err(|e| StorageError::ReadError(e.to_string()))?;
            let open_table = tx.open_table(table_def).map_err(|e| StorageError::ReadError(e.to_string()))?;
            let value = open_table.get(key).map_err(|e| StorageError::ReadError(e.to_string()))?;
    ...
    async fn delete(&mut self, table: String, key: u64) -> Result<(), StorageError> {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            {
                let mut open_table = tx.open_table(table_def).map_err(|e| StorageError::WriteError(e.to_string()))?;
                let value = open_table.remove(key).map_err(|e| StorageError::WriteError(e.to_string()))?;
    ...
    async fn update(&mut self, table: String, key: u64, value: Vec<u8>) -> Result<(), StorageError>
    {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let tx = db.read().begin_write().map_err(|e| StorageError::WriteError(e.to_string()))?;
            {
                ...
                let res = open_table.insert(key, value.as_slice()).map_err(|e| StorageError::WriteError(e.to_string()))?;
                if res.is_none() {
            ...
            tx.commit().map_err(|e| StorageError::CommitError(e.to_string()))?;
            Ok(())
        }).await.expect("Failed to spawn task").map_err(|e| StorageError::UpdateError(e.to_string()))
    }
    ...
    async fn upsert(&mut self, table: String, key: u64, value: Vec<u8>) -> Result<bool, StorageError> {
        ...
        let result = tokio::task::spawn_blocking(move || {
            ...
            tx.commit().map_err(|e| StorageError::WriteError(e.to_string()))?;
            Ok(false)
        }).await.expect("Failed to spawn task")?;
        Ok(result)
    }
    ...
    async fn flush(&mut self) -> Result<(), StorageError> {
        ...
        match tokio::task::spawn_blocking(move || { 
            db.write().compact().map_err(|e| StorageError::FlushError(e.to_string()))
        }).await.expect("Failed to flush database") {
            Ok(_) => Ok(()),
    ...
    async fn create_table(&mut self, table: String) -> Result<(), StorageError> {
        ...
        tokio::task::spawn_blocking(move || {
            let table_def: TableDefinition<u64, &[u8]> = TableDefinition::new(&table);
            {
                let tx = db.read().begin_write().map_err(|e| StorageError::TableError(e.to_string()))?;
                {
                    tx.open_table(table_def).map_err(|e| StorageError::TableError(e.to_string()))?;
                }
                tx.commit().map_err(|e| StorageError::CommitError(e.to_string()))?;
                Ok::<(), StorageError>(())
            }
