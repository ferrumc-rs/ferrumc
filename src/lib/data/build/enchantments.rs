...
pub(crate) fn build() -> TokenStream {
    ...
    let enchantments: BTreeMap<String, Enchantment> =
        serde_json::from_str(&fs::read_to_string("../../../assets/extracted/enchantments.json").unwrap())
    ...
    for (name, enchantment) in enchantments.iter() {
        let const_ident = format_ident!("{}", name.strip_prefix("minecraft:").unwrap_or(name).to_shouty_snake_case());
        let id_lit = LitInt::new(&enchantment.id.to_string(), Span::call_site());
        
        let translate = &enchantment.description.translate;
        let min_cost_base = LitFloat::new(&format!("{:.1}", enchantment.min_cost.base), Span::call_site());
        let min_cost_per_level = LitFloat::new(&format!("{:.1}", enchantment.min_cost.per_level_above_first), Span::call_site());
        let max_cost_base = LitFloat::new(&format!("{:.1}", enchantment.max_cost.base), Span::call_site());
        let max_cost_per_level = LitFloat::new(&format!("{:.1}", enchantment.max_cost.per_level_above_first), Span::call_site());
        ...
        let slots = enchantment.slots.iter().map(|slot| {
