...
impl SnowGolemBundle {
    pub fn new(position: Position) -> Self {
        let metadata = EntityMetadata::from_vanilla(&VanillaEntityType::SNOW_GOLEM);
...
mod tests {
    ...
    #[test]
    fn test_snow_golem_bundle_creation() {
        let position = Position::new(0.0, 64.0, 0.0);
        let snow_golem = SnowGolemBundle::new(position);

        assert_eq!(snow_golem.metadata.protocol_id(), 116);
        assert_eq!(snow_golem.metadata.resource_name(), "snow_golem");
        assert!(snow_golem.metadata.is_mob());
    }

    #[test]
    fn test_snow_golem_bundle_with_rotation() {
        let position = Position::new(10.0, 70.0, 20.0);
        let rotation = Rotation {
            yaw: 90.0,
            pitch: 0.0,
        };
        let snow_golem = SnowGolemBundle::with_rotation(position, rotation);
        assert_eq!(snow_golem.rotation.yaw, 90.0);
    }
}
