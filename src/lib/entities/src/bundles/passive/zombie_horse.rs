...
impl ZombieHorseBundle {
    pub fn new(position: Position) -> Self {
        let metadata = EntityMetadata::from_vanilla(&VanillaEntityType::ZOMBIE_HORSE);
...
mod tests {
    ...
    #[test]
    fn test_zombie_horse_bundle_creation() {
        let position = Position::new(0.0, 64.0, 0.0);
        let zombie_horse = ZombieHorseBundle::new(position);

        assert_eq!(zombie_horse.metadata.protocol_id(), 146);
        assert_eq!(zombie_horse.metadata.resource_name(), "zombie_horse");
        assert!(zombie_horse.metadata.is_mob());
    }

    #[test]
    fn test_zombie_horse_bundle_with_rotation() {
        let position = Position::new(10.0, 70.0, 20.0);
        let rotation = Rotation {
            yaw: 90.0,
            pitch: 0.0,
        };
        let zombie_horse = ZombieHorseBundle::with_rotation(position, rotation);
        assert_eq!(zombie_horse.rotation.yaw, 90.0);
    }
}
