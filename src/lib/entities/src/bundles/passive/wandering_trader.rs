...
impl WanderingTraderBundle {
    pub fn new(position: Position) -> Self {
        let metadata = EntityMetadata::from_vanilla(&VanillaEntityType::WANDERING_TRADER);
...
mod tests {
    ...
    #[test]
    fn test_wandering_trader_bundle_creation() {
        let position = Position::new(0.0, 64.0, 0.0);
        let wandering_trader = WanderingTraderBundle::new(position);

        assert_eq!(wandering_trader.metadata.protocol_id(), 136);
        assert_eq!(
            wandering_trader.metadata.resource_name(),
            "wandering_trader"
        );
        assert!(wandering_trader.metadata.is_mob());
    }

    #[test]
    fn test_wandering_trader_bundle_with_rotation() {
        let position = Position::new(10.0, 70.0, 20.0);
        let rotation = Rotation {
            yaw: 90.0,
            pitch: 0.0,
        };
        let wandering_trader = WanderingTraderBundle::with_rotation(position, rotation);
        assert_eq!(wandering_trader.rotation.yaw, 90.0);
    }
}
