...
impl VillagerBundle {
    pub fn new(position: Position) -> Self {
        let metadata = EntityMetadata::from_vanilla(&VanillaEntityType::VILLAGER);
...
mod tests {
    ...
    #[test]
    fn test_villager_bundle_creation() {
        let position = Position::new(0.0, 64.0, 0.0);
        let villager = VillagerBundle::new(position);

        assert_eq!(villager.metadata.protocol_id(), 134);
        assert_eq!(villager.metadata.resource_name(), "villager");
        assert!(villager.metadata.is_mob());
    }

    #[test]
    fn test_villager_bundle_with_rotation() {
        let position = Position::new(10.0, 70.0, 20.0);
        let rotation = Rotation {
            yaw: 90.0,
            pitch: 0.0,
        };
        let villager = VillagerBundle::with_rotation(position, rotation);
        assert_eq!(villager.rotation.yaw, 90.0);
    }
}
