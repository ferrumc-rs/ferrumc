...
use crate::bench_nbt_codec::nbt_codec::Root;
...
fn bench_serialize_nbt_codec(c: &mut Criterion) {
    let file = std::fs::read("./.etc/nbt_codec.nbt").unwrap();
    let dimension = fastnbt::from_bytes::<Root>(&file).unwrap();

    c.bench_function("serialize nbt_codec", |b| b.iter(|| {
        let mut buffer = Vec::with_capacity(1024);
        dimension.serialize(&mut buffer).unwrap();
        buffer
    }));
}

criterion_group!(benches, bench_serialize_nbt_codec);

criterion_main!(benches);


// 0, 22 ( 0 < x < 22 ) 23
// 15, 13 ( x>15, 0<x<13 ) 14
