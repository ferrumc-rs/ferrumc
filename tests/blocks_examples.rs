...
fn test_blocks_shapes() {
    ...
    if let Some(first_shape) = shapes.first() {
        assert!(first_shape.min.x <= first_shape.max.x);
        assert!(first_shape.min.y <= first_shape.max.y);
        assert!(first_shape.min.z <= first_shape.max.z);
    }
}
...
fn test_blocks_comprehensive_demo() {
    ...
    for (name, block) in interesting_blocks {
        assert!(block.id >= 0, "{} should have valid ID", name);
        assert!(!block.name.is_empty(), "{} should have non-empty name", name);
        assert!(block.hardness >= 0.0 || block.hardness == -1.0, "{} should have valid hardness", name);
        assert!(block.blast_resistance >= 0.0, "{} should have valid blast resistance", name);
    }
}
...
fn test_blocks_edge_cases() {
    ...
    // Test invalid ID
    assert!(blocks::Block::by_id(-1).is_none());
    assert!(blocks::Block::by_id(i32::MAX).is_none());
    ...
    let stone_lower = blocks::Block::by_name("stone");
    let stone_upper = blocks::Block::by_name("STONE");
    let stone_mixed = blocks::Block::by_name("Stone");
    
    assert!(stone_lower.is_some());
