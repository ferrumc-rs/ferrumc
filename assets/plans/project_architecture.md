...
## **Layer 2: The Engine State (ECS Schema)**

*These crates define how the game state is represented in memory. They depend on Layer 1 \+ Bevy.*

### **ferrumc-ecs**

* **Purpose:** The single source of truth for the ECS schema. Merges Components and Messages to prevent circular dependencies.  
* **Contents:**  
  * **Components:** Position, Health, Inventory, NetworkConnection (holds Box\<dyn PacketSender\>).  
  * **Messages (Events):** PlayerJoin, BlockBreak, Chat, EntityMove.  
...
## **Layer 3: The Interfaces**

*These define the public contracts for external communication. Depends on Layer 1\.*
...
* **Critical:** Depends on ferrumc-core (for Chunk structs), but **NOT** on ferrumc-storage or ferrumc-net.
...
* **Dependency Rule:** Depends on ferrumc-core, ferrumc-ecs, ferrumc-registry. Does **NOT** depend on net or storage directly (uses Traits).
...
* **Contents:**  
  ...
  4. **Registration:** Adds ferrumc-plugins to the App.  
