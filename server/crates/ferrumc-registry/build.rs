...
fn main() {
    ...
    // Derived per-state collision-top heights live beside the checksummed vendored
    // fixtures (integrity-guarded by `manifest.toml`), not in `data/`, so there is a
    // single provenance-tracked copy. Regenerated by `scripts/gen_collision_heights.py`.
    let collision_path =
        manifest_dir.join("../../../fixtures/protocol/1_21_8/block_collision_heights.json");
...
struct Block {
    ...
    /// `fixtures/protocol/1_21_8/block_collision_heights.json` (derived from the pinned
    /// `PrismarineJS` `blockCollisionShapes` by `scripts/gen_collision_heights.py`); a
    /// block absent from that file falls back to the coarse `boundingBox` (`1.0` for a
    /// full cube, `0.0` for none).
