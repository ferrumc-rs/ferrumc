...
- **Falling blocks (gravity), animated end-to-end.** A gravity-affected block (sand, red sand, gravel, all concrete powders, anvils, dragon egg, pointed dripstone) that loses support on place or break falls on the owning shard: the unsupported block becomes a simulated falling entity, drops under per-category gravity + air drag, lands on the first solid block below, and restores its block at the resting cell. Breaking the base of a stack collapses the whole gravity column at once and it re-stacks on landing. Runs deterministically in `ferrumc-sim` (fixed-order per-tick step); an entity that falls past y=-128 (64 blocks below the world floor) despawns instead of falling forever. The session layer now draws the fall: the shard's `EntitySpawned` / `EntityMoved` / `EntityDespawned` outputs are translated to a clientbound `SpawnEntity` (`minecraft:falling_block`, type 49, carrying the block-state), per-tick position updates, and `RemoveEntities` on landing — scoped to viewers within view distance, with a fresh network entity id per entity so it never collides with a player's.
  - *Test:* Place sand/gravel with nothing under it, or break the block supporting a sand pillar — you see it **fall** as an animated block and settle on the ground (no more teleport-jump). Stack several sand on a one-block pillar and break the pillar; the whole stack falls and re-stacks. A second player in range sees the same animation.
- ⚠ **Non-block gravity entities still have no spawner.** The physics step and the session-side entity rendering can carry any entity (dropped items, primed TNT, projectiles, XP orbs), but nothing produces them yet — only falling blocks feed the pipeline. A generic (`Simple`) spawn has no wire entity type assigned, so it is simulated but not drawn.
  - *Test:* Break a block — no item entity drops (creative has no drops anyway); there is no TNT/arrow/orb to observe.
